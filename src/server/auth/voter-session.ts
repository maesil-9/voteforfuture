 ...
 * 유권자 세션.
 *
 * - 유권자는 계정이 없다. 유효한 코드를 입력하면 짧은 TTL의 서명된
 *   httpOnly 쿠키를 발급한다.
 * - 쿠키에는 election id와 code hash(서명됨)만 담는다. 코드 원문은 담지 않는다.
...
export async function createVoterSession(electionId: string, codeHash: string) {
  await writeSession({
    electionId,
    codeHash,
